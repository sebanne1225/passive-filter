...
- 2026-06-19: テンプレから scaffold → annotated-plan-flow（research/plan は `plans/001_passive-filter-mvp/`、git 外）→ **MVP 実装完了（T1〜T8）**。
  ...
  - **compile 検証 PASS**（Unity MCP: `Packages/manifest.json` に file 参照追加 → force refresh → 0 errors / 0 warnings、4 型コンパイル確認）。
  - **未**: テストアバターでの実機ビルド挙動確認（要実機確認 = Skinned/Light の m_Enabled / ParticleSystem emission の反映 / curve value 読み）。commit も未（明示指示待ち）。
- 次の一手: 実機ビルド挙動の検証（テストアバターにトグル各種を仕込み → コンポーネント付与 → NDMF ビルド → 初期非表示/メニュー操作で出せるかを確認）→ 問題なければ commit → リリース準備。
