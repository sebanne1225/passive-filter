...
- **2026-06-22: 1.0.0 初回リリース完了**（tag 1.0.0 / GitHub Release / VPM listing / VCC / BOOTH 全公開・repo public 化）。BOOTH: https://sebanne.booth.pm/items/8530834
  - TD-P-T9: (A) clean N子変種補正を合成アバター（3子 Simple1D off/on-A/on-B）+ NDMF ベイクで **runtime 実証（PASS）**。TD-P-T4 の (A) 未実証を解消。
  - verbose log toggle: `PassiveFilterSettings.verboseLogging`（既定 OFF）で詳細 Info ログをゲート（`PassiveFilterLog.Verbose` を各ビルドパス先頭でセット）。NDMF ErrorReport（補正不可通知）は常時表示で不変。
  - 検証 MA version = **1.17.1**。死にコード所見: `PassiveFilterLog.Warn/Error` 未使用（プロジェクト規約により保持）。
  - 詳細はセッション記録 DB「Passive Filter 1.0.0 初回リリース完了」を参照（変動値は CLAUDE.md に持たない）。
